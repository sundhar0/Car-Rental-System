...
function LoginPage() {
  ...
  const login = () => {
    ...
    axios
      .post("http://localhost:8080/api/userLogin/token/generate", body)
      .then((response) => {
        ...
        axios
          .get("http://localhost:8080/api/userLogin/userDetails", {
            headers: {
    ...
    <div className="login-wrapper d-flex justify-content-center align-items-center vh-100">
      <div className="login-card text-center p-4">
        ...
        <div className="card-body text-white small">
          {msgUsername == null ? "" : <div className="mb-4">{msgUsername}</div>}
          {msgPassword == null ? "" : <div className="mb-4">{msgPassword}</div>}

          <div className="text-start mb-3">
            <label className="form-label text-success small">
              Enter Your Username
            </label>
            <input
              type="email"
        ...
        <Link to="/becomedriver" className="btn btn-outline-success w-100">
          Sign up
        </Link>
      </div>
    </div>
